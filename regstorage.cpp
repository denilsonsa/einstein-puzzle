...
int RegistryStorage::get(const std::wstring &name, int dflt)
{
    ...
    if (globalKey) {
        ...
        DWORD type;
        if (! RegQueryValueExA(globalKey, uname.c_str(), NULL, &type,
                    (BYTE*)&data, &size))
            if (type == REG_DWORD)
                return data;
    }
    ...
    if (userKey) {
        ...
        DWORD size = sizeof(data);
        DWORD type;
        if (! RegQueryValueExA(userKey, uname.c_str(), NULL, &type,
                    (BYTE*)&data, &size))
            if (type == REG_DWORD)
                return data;
    }
...
void RegistryStorage::set(const std::wstring &name, int value)
{
    ...
    if (userKey) {
        DWORD data = value;
        RegSetValueExA(userKey, uname.c_str(), 0, REG_DWORD, 
...
void RegistryStorage::set(const std::wstring &name, const std::wstring &value)
{
    ...
    if (userKey)
        RegSetValueExA(userKey, uname.c_str(), 0, REG_SZ, 
